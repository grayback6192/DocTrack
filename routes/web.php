...
Route::get('admin/{upgid}/department/add',function($upgid){
	...
	return view('admin/addDep',['motherGroups'=>$motherGroups,'User'=>$name,'upgid'=>$upgid,'admingroup'=>$admingroup]);
})->name('regDep');
...
Route::get('admin/{upgid}/assignment', function($upgid){
	...
	else
	{
		...
		$allupgs = DB::table('userpositiongroup')->where('group_group_id','=',$admingroup)->where('upg_status','=','active')->get();
	}
