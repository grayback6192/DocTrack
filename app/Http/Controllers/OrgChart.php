...
class OrgChart extends Controller
{
    ...
    public function update(Request $request,$upgid)
    {
        ...
        $removeorgchartnodesarray = $request['removenodearray'];
        $orgchartid = $request['orgchartid'];
        $count = count($removeorgchartnodesarray);
        $orgchartnodesedit = $request['orgchartnodesArrayEdit'];
        $count1 = count($orgchartnodesedit);
    \DB::table('orgchart')->where('group_id','=',$group)->update(['path'=>$filename.'orgchart.txt']);


    if(isset($removeorgchartnodesarray)==!null){
      var_dump($removeorgchartnodesarray);
       foreach ($removeorgchartnodesarray as $nodesArrayDel) {
         $upg_id_del = $nodesArrayDel;
        \DB::table('orgchartnode')->where('orgchart_id','=',$orgchartid)
                                  ->where('upg_id','=',$upg_id_del)
                                  ->delete();
       }
    }
    
         if(isset($orgchartnodesedit)==!null){
                foreach ($orgchartnodesedit as $nodesArray) {
                  $nodeid = $nodesArray['nodeid'];
                  $pos_id = $nodesArray['pos_id'];
                  $prevupgid = $nodesArray['prevupgid'];
                  $upg_id = $nodesArray['upg_id'];
                  $groupid = $nodesArray['group_id'];
                  $orgchartid = $nodesArray['orgchart_id'];
                  \DB::table('orgchartnode')->where('orgchart_id','=',$orgchartid)
                                             ->where('upg_id','=',$prevupgid)
                                             ->update(['orgchartnode_id'=>$nodeid,
                                                       'orgchart_id'=>$orgchartid,
                                                       'pos_id'=>$pos_id,
                                                       'upg_id'=>$upg_id,
                                                       'group_id'=>$groupid]);
    ...
    if(isset($orgchartnodesarrays)==!null)
