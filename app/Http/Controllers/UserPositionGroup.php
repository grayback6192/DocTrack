...
class UserPositionGroup extends Controller
{
    ...
    public function addNewAssignment(Request $request,$upgid)
    {
        ...
        $orgchartParent = DB::table('deppos as dp')->where('pos_id','!=','12345')->where('motherPos','=',$request['position'])->join('userpositiongroup as upg','dp.deppos_id','upg.position_pos_id')->where('user_user_id','!=','999999')->get();
        // dd($request['position']);
...
//finding undefine name and changing it
 ...
 return redirect()->route('showDep',['upgid'=>$upgid,'id'=>$request['group']]);
    ...
    public function enterGroup(Request $request)
    {
        ...
        $studposStud = DB::table('position as p')->where('p.posName','=','Student')
                                                 ->where('client_id','=',$request['clientid'])
                                                 ->value('pos_id');
        $studdeppos = DB::table('deppos as dp')->where('dp.pos_id','=',$studposStud)->value('deppos_id');
...
// dd($request['groupid']);
        ...
        else if($request['position']=="Student")
        {
            ...
            if(count($found)==1){
                // dd($request->all());
             DB::table('userpositiongroup')->where('user_user_id','=',$user->user_id)
            ->update(['group_group_id'=>$request['groupid'],'position_pos_id'=>$studdeppos, 'upg_status'=>'active']);
        }else if(count($found)==0){

            
            DB::table('userpositiongroup')->insert(['upg_id'=>$rand,
                                                    'position_pos_id'=>$studposStud,
                                                    'rights_rights_id'=>2,
                                                    'user_user_id'=>$user->user_id,
                                                    'group_group_id'=>$request['groupid'],
                                                    'client_id'=>$request['clientid'],
