...
class LoginController extends Controller
{
    ...
    */
    ...
    protected function redirectTo()
    {
        ...
        //for log in as
        $usergroups = \DB::table('userpositiongroup as upg')->where('upg.upg_id','=',$upgid)
        ...
        if(count($usergroups)>0)
        {
            foreach ($usergroups as $usergroup) {
                        $positionName = $usergroup->posName;
                        $groupName = $usergroup->groupName;
                    }

                      \Session::put('posName',$positionName);
            \Session::put('groupName',$groupName);
        }
                    
            \Session::put('client',$clientid);
            \Session::put('userUPGID',$upgid);
