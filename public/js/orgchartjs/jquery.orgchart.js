...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    loopChart: function ($chart) {
      ...
      var subObj = { 'name': $tr.find('.title').text(), 'upgid': $tr.find('input').val(), 'positions': $tr.find('.upgid').text()}
    ...
    createNode: function (nodeData, level, opts) {
      ...
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeContent] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      ...
      if (opts.nodeTemplate) {
        ...
        $nodeDiv.append('<div id="'+nodeData[opts.nodeTitle]+'" class="title">' + nodeData[opts.nodeTitle] + '</div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content"><div id="node-upgid" class="upgid">' + (nodeData[opts.positions] || '') + '</div> <input class="nodeupgid" type = "hidden" value = "'+(nodeData[opts.nodeContent] || '') +'"></div>' : '');
      }
    ...
    removeNodes: function ($node) {
      ...
      if ($parent.is('td')) {
        ...
        if (this.getNodeState($node, 'siblings').exist) {
          ...
          $parent.remove();
        }else if(this.getNodeState($node,'children').exist){
          alert('You need to delete child node first');
        }
         else {
